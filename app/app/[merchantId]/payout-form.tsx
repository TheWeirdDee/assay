...
export function PayoutForm({ merchantId }: { merchantId: string }) {
  ...
  return <div className="flex min-w-0 flex-col gap-4 rounded-2xl border border-zinc-700 bg-zinc-900 p-5 shadow-sm sm:p-6">
    ...
    <form action={formAction} className="grid min-w-0 gap-4 md:grid-cols-[minmax(0,1fr)_9rem] xl:grid-cols-[minmax(0,1fr)_9rem_auto] xl:items-end">
      ...
      <label className="flex min-w-0 flex-col gap-2 text-sm font-semibold text-zinc-300">Amount<input name="amount" type="number" step="0.01" min="0" required className={`${input} min-w-0 w-full`} /></label>
