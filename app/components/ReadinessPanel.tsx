...
export function ReadinessPanel({ readiness, merchantId }: { readiness: MerchantReadiness; merchantId: string }) {
  const items = [
  ...
  return <section className={`rounded-2xl border p-5 ${ready ? "border-emerald-800/50 bg-emerald-950/15" : "border-amber-800/50 bg-amber-950/15"}`}>
    ...
    {!ready && <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-zinc-300">{readiness.identity !== "ready" && <VerifyIdentityButton merchantId={merchantId} />}<span>Fund the wallet with testnet MON and aUSDC to enable settlement.</span></div>}
  </section>;
}
